...
class AttendanceController extends Controller
{
    ...
    {
        $atts = Attendance::with('work')
            ->join('work__schedules', 'attendances.work_id', '=', 'work__schedules.id')
            ->where('present', 1)
            ->orderBy('work__schedules.date', 'DESC')
            ->get();

        $absatts = Attendance::with('work')
            ->join('work__schedules', 'attendances.work_id', '=', 'work__schedules.id')
            ->where('present', 0)
            ->orderBy('work__schedules.date', 'DESC')
            ->get();
    ...
    public function clock_out(Request $request, string $id)
    {
        ...
        $att->description = $att->description . "-" . $status;
